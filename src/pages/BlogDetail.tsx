  ...
  if (!post) {
    return (
      <section className="container-studio py-32 text-center">
        ...
        <Button asChild variant="hero" className="mt-6"><Link to="/blog">Back to journal</Link></Button>
      </section>
    );
  }
  ...
  return (
    <>
      <article className="container-studio max-w-3xl pt-20 pb-16">
        <Link to="/blog" className="text-sm text-muted-foreground hover:text-foreground transition-colors">← Journal</Link>
        ...
        <div className="aspect-[16/9] mt-12 rounded-2xl bg-gradient-to-br from-primary/20 via-secondary to-accent-purple/20 border border-border grid place-items-center relative overflow-hidden">
          ...
          <span className="relative font-display text-7xl font-bold text-gradient">{post.title.charAt(0)}</span>
        </div>
        ...
        <div className="mt-16 pt-8 border-t border-border flex items-center justify-between">
          ...
          <Button asChild variant="outline" size="sm"><Link to="/blog">More posts</Link></Button>
        </div>
      </article>
